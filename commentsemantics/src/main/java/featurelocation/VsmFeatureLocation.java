...
public class VsmFeatureLocation implements FeatureLocation 
{
	...
	{	
		...
		Field field2 = new Field("artefact", String.join(" ", artefact), fieldType);
		Field field3 = new Field("comments", String.join(" ", comments), fieldType);
	...
	void printSimilarDocuments(List<String> javafiles, String scope) throws IOException
	{
		System.out.println("\nTextual feature location - scope:" + scope);
		System.out.println("******************************************");
		...
		List<Integer> inputfileIndex = new ArrayList<Integer>();

		for (int i = 0; i < reader.numDocs(); i++) 
		{
			Terms vector = reader.getTermVector(i, scope);   //comments / artifact
			realVectors.add(getRealVector(vector, allTerms, reader, scope));
		
			for(String fileName : javafiles) {   //find the index of each file name
			
				if (reader.document(i).getField("filename").stringValue().compareToIgnoreCase(fileName) == 0)
					inputfileIndex.add(i);
			}			
		}
		
		List<String> similarDocuments = new ArrayList<String>();
		
		for (Integer fileindex : inputfileIndex) 
		{
	...
	static RealVector getRealVector(Terms vector, ArrayList<String> allTerms, IndexReader reader, String field) throws IOException 
	{
		...
		while(terms.next() != null) 
		{
			...
			Term term = new Term(field, terms.term().utf8ToString() );
	...
	static ArrayList<String> getAllTerms(IndexReader reader, String scope) throws IOException 
	{
		Set<String> allTerm = new HashSet<String>();

		for(int i=0;i<reader.numDocs();i++) 
		{
			Terms vector = reader.getTermVector(i, scope);
